export ExtremeValueDecision, extreme_value_decision, demeaned, draw_gumbel_shocks
...
function extreme_value_decision(d :: ExtremeValueDecision{F}, value_iV :: Vector{F}) where
    ...
    if !demeaned(d)
        valueV .+= (d.prefScale * F(EulerConst));
    end
    return probV, valueV
end
...
function extreme_value_decision(d :: ExtremeValueDecision{F}, 
   value_ixM :: Matrix{F}) where  F <: AbstractFloat
    ...
    # Expected value
    eVal_iV = vec(d.prefScale .* (vMax_iV + log.(expSum_iV)));

    if !demeaned(d)
      eVal_iV .+= d.prefScale * EulerConst;
    end
